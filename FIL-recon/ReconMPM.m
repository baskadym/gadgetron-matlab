...
function ReconMPM(connection)
    ...
    w                = 'auto'; %6;           % Smoothing Kernel Size. Use 'auto' if unsure; this will display a suggested 'w' during image reconstruction, which can then be fixed here or modified during optimisation process.
    N_ref            = 6; %[6 1]             % Number of outer products to use for svd to estimate sensitivity
    
    sens_grad_scale  = 1;                    % Sensitivity gradient scaling -- relative contribution to 2nd SVD
    lambda           = 'auto'; %3e-4;        % Regularisation factor for pseudo inversion. Use 'auto' if unsure; this will display a suggested lambda during image reconstruction, which can then be fixed here or modified during optimisation process.
